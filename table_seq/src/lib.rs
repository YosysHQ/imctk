...
//!
//! For sufficiently sparse but large sequences of tables, using [`TableSeq`] only requires a
...
//! subtable index part of the key, [`TableSeq`] has two major advantages:
//!
//! * It maintains locality of subtables, which can result in a large performance advantage for use
//!   cases where consecutive accesses are typically to the same subtable or to the same small set
//!   of subtables. Using a single hash table would spread those accesses randomly across all the
//!   full table, hitting a performance cliff each time the full table size exceeds the size of a
//!   cache level. With an [`TableSeq`] this only happens with the combined size of currently
//!   "hot" subtables exceeds the cache size.
//!
//! * An [`TableSeq`] provides methods for iterating over individual subtables. Such methods
...
//! regards to iteration over subranges, [`TableSeq`] provides better performance for large
...
//! subtable is a separate tree, [`TableSeq`] tends to have much lower memory usage when many
//! subtables have only one or two entries, as this will waste a lot of memory in mostly empty
//! B-tree root nodes. Note that the optimizations that [`TableSeq`] uses here are not specific
...
//!
//! Internally, an [`TableSeq`] groups subtables into chunks of 16 consecutive subtables
...
//!
//! [^1]: Due to a technicality we add allocators in pairs and thus require two allocators for every
//!     span of `1 << 21` subtables.
//!
...
//! improvement, as this fragmentation does not persist beyond the [`TableSeq`]'s lifetime, but
